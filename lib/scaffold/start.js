...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for suqacored is necessary for this upgrade with the "reindex=1" SUQA.conf option. \n' +
                 'There are changes necessary in both SUQA.conf and suqacore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/crptec/suqacore-node/blob/suqad/docs/upgrade.md \n');
        ...
        servicesConfig: {
          suqacored: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/suqacored')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed suqacore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './suqacore-node.json');
...
function loadModule(req, service) {
  ...
  try {
    // first try in the built-in suqacore-node services directory
    service.module = req(path.resolve(__dirname, '../services/' + service.name));
  } catch(e) {
    console.log("Load built-in module: " + e);
	console.log("Load add-on module...");
    ...
    if (servicePackage.SUQAcoreNode) {
      serviceModule = service.name + '/' + servicePackage.SUQAcoreNode;
    }
 ...
 * @param {Node} node - The SUQAcore Node instance
 */
