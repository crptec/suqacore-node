...
/**
 * Will remove a service from suqacore-node.json
...
/**
 * Will remove the Node.js service and from the suqacore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The suqacore-node configuration directory
...
function remove(options, done) {
  ...
  var suqacoreConfigPath = path.resolve(configPath, 'suqacore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(suqacoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a suqacore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to suqacore-node.json
        removeConfig(suqacoreConfigPath, service, next);
      });
    }, done
  );
}
