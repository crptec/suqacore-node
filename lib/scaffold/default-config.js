...
/**
 * Will return the path and default suqacore-node configuration. It will search for the
 * configuration file in the "~/.suqacore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.suqacore');
  var defaultConfigFile = path.resolve(defaultPath, './suqacore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['suqacored', 'web'];
