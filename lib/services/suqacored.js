...
/**
 * Provides a friendly event driven API to suqacored in Node.js. Manages starting and
 * stopping suqacored as a child process for application support, as well as connecting
 * to multiple suqacored processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function SUQA(options) {
  if (!(this instanceof SUQA)) {
    return new SUQA(options);
  }
...
util.inherits(SUQA, Service);

SUQA.dependencies = [];

SUQA.DEFAULT_MAX_TXIDS = 1000;
SUQA.DEFAULT_MAX_HISTORY = 50;
SUQA.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
SUQA.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
SUQA.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
SUQA.DEFAULT_SPAWN_RESTART_TIME = 5000;
SUQA.DEFAULT_SPAWN_STOP_TIME = 10000;
SUQA.DEFAULT_TRY_ALL_INTERVAL = 1000;
SUQA.DEFAULT_REINDEX_INTERVAL = 10000;
SUQA.DEFAULT_START_RETRY_INTERVAL = 5000;
SUQA.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
SUQA.DEFAULT_TRANSACTION_CONCURRENCY = 5;
SUQA.DEFAULT_INSTANTSEND_FEE = 10000;
SUQA.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'suqarpc',
  rpcpassword: 'BEAtr3Cw8yrTfTuic63CGcrKPfJj7UxZMmamYXapLGue',
  uacomment: 'suqacore'
};

SUQA.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || SUQA.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || SUQA.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || SUQA.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || SUQA.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || SUQA.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || SUQA.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || SUQA.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || SUQA.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || SUQA.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || SUQA.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

SUQA.prototype._initCaches = function() {
...
SUQA.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
SUQA.prototype.getAPIMethods = function() {
  var methods = [
...
SUQA.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'suqacored/' + name, 'total:', this.subscriptions[name].length);
};

SUQA.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'suqacored/' + name, 'total:', this.subscriptions[name].length);
};

SUQA.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (suqacore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'suqacored/addresstxid', 'total:', _.size(this.subscriptions.address));
};

SUQA.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'suqacored/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
SUQA.prototype.unsubscribeAddressAll = function(emitter) {
...
SUQA.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = SUQA.DEFAULT_CONFIG_SETTINGS;
...
SUQA.prototype._parseDashConf = function(configPath) {
  var options = {};
...
SUQA.prototype._expandRelativeDatadir = function() {
...
SUQA.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in suqacored config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in suqacored config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in suqacored config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './SUQA.conf');

  log.info('Using SUQA config file:', configPath);
...
SUQA.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of suqacore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of suqacore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of suqacore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in SUQA.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that suqacored is undergoing a reindex. ' +
             ...
             'of suqacore-node services will start.');
    node._reindex = true;
  }
};

SUQA.prototype._resetCaches = function() {
 ...
 * @private
 */
SUQA.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
SUQA.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

SUQA.prototype._initChain = function(callback) {
  var self = this;
...
SUQA.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 20970
  };
  if (this.node.network === suqacore.Networks.testnet) {
    networkOptions.rpcport = 20980;
  }
  return networkOptions;
};

SUQA.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === suqacore.Networks.testnet) {
    networkPath = 'testnet3/SUQA.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/SUQA.conf';
    }
  }
  return networkPath;
};

SUQA.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === suqacore.Networks.testnet) {
...
SUQA.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('suqacored/hashblock', message.toString('hex'));
    }
  }

};

SUQA.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
SUQA.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('SUQA Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
SUQA.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
SUQA.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('suqacored/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
SUQA.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = suqacore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('suqacored/rawtransaction', message.toString('hex'));
    }

    var tx = suqacore.Transaction();
    tx.fromString(message);
    var txid = suqacore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

SUQA.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = suqacore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('suqacored/transactionlock', message.toString('hex'));
    }

  }
};

SUQA.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || SUQA.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

SUQA.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
SUQA.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
SUQA.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('SUQA Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || SUQA.DEFAULT_REINDEX_INTERVAL);

};

SUQA.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
SUQA.prototype._stopSpawnedSUQA = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/suqad.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned SUQA process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean SUQA process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
SUQA.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedSUQA(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting SUQA process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('SUQA process unexpectedly exited with code:', code);
        log.warn('Restarting SUQA child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('SUQA process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn suqacored.'));
      }
...
SUQA.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to suqacored.'));
    }
 ...
 */
SUQA.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('SUQA configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
SUQA.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
SUQA.prototype.syncPercentage = function(callback) {
  var self = this;
...
SUQA.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, suqacore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
SUQA.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = suqacore.Script.fromAddress(delta.address);
    return {
...
SUQA.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
SUQA.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
SUQA.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in suqacore is most recent to less recent
 ...
 * @param {Function} callback
 */
SUQA.prototype.getAddressTxids = function(addressArg, options, callback) {
  ...
  function finish() {
    if (txids && !rangeQuery) {
      ...
      self.client.getAddressTxids(txidOpts, function(err, response) {
		console.log("suqacored: client.getAddressTxids : client size : " + self.client.length + JSON.stringify(err));  
        if (err) {
          return callback(self._wrapRPCError(err));
        }
...
SUQA.prototype._getConfirmationsDetail = function(transaction) {
...
SUQA.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
SUQA.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
SUQA.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A SUQA transaction id
 * @param {Function} callback
 */
SUQA.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
SUQA.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof suqacore.Address) {
...
SUQA.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
SUQA.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
SUQA.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
  ...
  console.log("suqacored: _normalizeAddressArg : " + JSON.stringify(addresses)); 
    ...
    async.parallel([
      function getTxList(done) {
        self.getAddressTxids(addresses, {queryMempool: false}, function(err, txids) {
		  console.log("suqacored: getAddressTxids error: " + JSON.stringify(err));	
          if (err) {
            return done(err);
          }
          summaryTxids = txids;
      ...
      function getBalance(done) {
        self.getAddressBalance(addresses, options, function(err, data) {
		  console.log("suqacored: getAddressBalance error: " + JSON.stringify(err));	 
          if (err) {
            return done(err);
          }
      ...
      function getMempool(done) {
        ...
        self.client.getAddressMempool({'addresses': addresses}, function(err, response) {
		  console.log("suqacored: getAddressMempool error: " + JSON.stringify(err));	
          if (err) {
	  ...
	  console.log("suqacored: parallel error: " + JSON.stringify(err)); 	
      if (err) {
        return callback(err);
      }
 ...
 * @private
 */
SUQA.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
SUQA.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a SUQA object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
SUQA.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = suqacore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
SUQA.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
SUQA.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
SUQA.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
SUQA.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
SUQA.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a SUQA Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
SUQA.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= SUQA.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new suqacore.Transaction(result.hex);
      var hash = suqacore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.suqacored.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
SUQA.prototype.govObjectList = function(options, callback) {
  var self = this;
...
SUQA.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
SUQA.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
SUQA.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
SUQA.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
SUQA.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
SUQA.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
SUQA.prototype.govObjectInfo = function (callback) {
  var self = this;
...
SUQA.prototype.govCount = function (callback) {
  var self = this;
...
SUQA.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
SUQA.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
SUQA.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
SUQA.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
SUQA.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.getInfo = function(callback) {
  var self = this;
...
SUQA.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
SUQA.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('suqacored spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('suqacored process did not exit'));
      }
